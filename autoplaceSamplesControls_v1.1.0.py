...
##P_VERSION:	1.0.0
##		1.1.0 added parameters --containerName --containerType for selection of output container
##		      added -x parameter for exluded artifact (i.e -x '<name>,<name>') and only exluded
##		      artifacts are treated as "control". Controls samples are sorted as N, P1, P2 at end
###############################################
...
def createContainer( type, name ):
	...
	## cType changes on server shift. Please check containerTypes xml
        ...
        cType = args[ "containerType" ]
        cTypeName = args[ "containerName" ]

        xml ='<?xml version="1.0" encoding="UTF-8"?>'
        xml += '<con:container xmlns:con="http://genologics.com/ri/container">'
        xml += '<name>' + cTypeName + '</name>'
...
def getRepCount():
	...
	global conRepNum	
        ...
        for key in I2OMap:
			...
			## check if its an exclude
			## check if artifact is in the excludeArr
                        isExclude = 0
                        for ex in excludeArr:
                        	isExclude = re.search( ex, oName )
                                if isExclude:
                                        break
...
def sortClinical():
	...
	for id in limsidArr_clinical:
		## gather sample information
		sampleNamePtr = re.search("\s?([A-Z]{1,3})\s?[-_]\s?(\d{1,2})\s?[-_]\s?([A-Z]{1,2})\s?[-_]\s?(\d{1,5})", nameDict[id])
		...
		## search for _1, _2 ... _n
		sampleRepPtr = re.search("[a-zA-Z]+\_(\d{1,2})", nameDict[id])
                sampleRep = sampleRepPtr.group(1)

		## place in values
		sampleDict[sampleOri][sampleYr][sampleType][sampleNum][sampleRep] = id
...
def sortCtrl():
	global limsidArr_ctrl
	
	## control arr
	ctrlDict = defaultdict(make_dict)

	for id in limsidArr_ctrl:
		num = re.search( "(Control)\s([0-9]{1,2})", nameDict[id] )
		if num:
			ctrlDict[nameDict[id]][num.group(2)] = id
		else:
			ctrlDict[nameDict[id]][0] = id
...
def autoPlace():
	...
	bufferRepNum = 1
        ...
        containerType, containerName = createContainer( '96', '96 well plate' )
	...
	## get user selected replication number
        ...
        for key in I2OMap:
			...
			## determine replicateNumber by replicate number appended behind artifact name by LIMS
				...
				isCtrl = re.search("Control", oName)
				
				if isClinical:
					limsidArr_clinical.append(output)

				elif isCtrl:
					limsidArr_ctrl.append(output)
				else:
					iWPArr.append(iWP)
	...
	if len(limsidArr_ctrl) > 0:
		## sort control samples
		sortDict = sortCtrl()
		
		for key in sorted(sortDict):
			for key2 in sorted(sortDict[key]):
				sortedLimsid.append(sortDict[key][key2])
...
def main():
	...
	hostname = ""
        ...
        opts, extraparams = getopt.getopt(sys.argv[1:], "l:u:p:s:x:", ["aCutOff=","aAlphabet=","aOffset=","cCutOff=","cAlphabet=","cOffset=","containerName=","containerType="])
		...
		## information available on <HOSTNAME>.gis.a-star.edu.sg:8080/api/v2/containertypes
		elif o == '--containerName':
			args[ "containerName" ] = p
		elif o == '--containerType':
			args[ "containerType" ] = p
